...
source(paste0('./www/fsecrets.R')) # import authentication variables
...
sc_players <- function(sc, round=NULL){
  print_log(paste0('sc_players: Round ', round))
  
  if(is.null(round)) round <- sc$var$current_round
  url <- paste0(sc$url$players, round)
  data <- sc_download(sc$auth, url)
