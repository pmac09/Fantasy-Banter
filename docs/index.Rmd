...
power_rankings <- function(player_data, sml_team_colours_hc, vPos=NULL){
  ...
  pRanks <- pRanks %>%
    ...
    arrange(coach, round) %>%
    group_by(coach) %>%
    mutate(avg3 = round(rollapply(score,width=7,sum,align='right',partial=TRUE)/
                        rollapply(n,7,sum,align='right', partial=TRUE),1)) %>%
    ungroup()
...
for(tid in 1:length(trades)){
  ...
  hc_trades[[tid]] <- highchart() %>%
    ...
    hc_add_series(name='Gomes on Field', data=hc_trade$played, yAxis=1, type='spline', lineWidth=0)
