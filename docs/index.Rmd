...
# for(i in 1:length(x)){
#   t <- rbindlist(x[[i]][["team_trade_players"]])
...
#               summarise(trade_avg = mean(points)), by=c('player_id','coach')) %>%
#   mutate(trade_id = ifelse(trade_id==75773,75731,trade_id)) %>%
#   mutate(diff = round(trade_avg - avg,1)) %>%
#   arrange(trade_id, coach, desc(avg))
# 
# 
# hc_trade <- trade_data2 %>%
#   filter(trade_id == unique(trade_data2$trade_id)[1])
# 
# highchart() %>%
#   hc_chart(type='column') %>%
#   hc_xAxis(categories= hc_trade$last_name,
#            plotBands=list(list(from=-0.5,to=0.5, label=list(text='Lester')),
#                           list(from= 0.5,to=2.5, color='#FFFFEF',label=list(text='Simon')))) %>%
#   hc_tooltip(shared=TRUE) %>%
#   hc_legend(enabled=FALSE) %>%
#   hc_add_series(data=hc_trade$avg, pointPlacement=0.1) %>%
#   hc_add_series(data=hc_trade$trade_avg, pointPlacement=-0.1) 
#   
