...
rankings_data <- player_data %>%
  ...
  mutate(score = round(points/n*18)) %>%
  arrange(coach, round) %>%
  group_by(coach) %>%
  mutate(avg3 = round(rollapply(score,3,mean,align='right',partial=TRUE),1)) %>%
  ungroup() %>%
...
highchart() %>%
  ...
  hc_subtitle(text='3 Round Average') %>%
  hc_exporting(enabled=TRUE) %>%
